...
Accountv2::Accountv2(std::string name, double balance)
    :name{name},balance{balance} {
        // std::cout << "[STATUS]: Constructor called -- Accountv2" << std::endl;
...
std::ostream &Accountv2::print(std::ostream &out) const {
    out << "[ACCOUNTV2] name: " << this->name << ", balance: $" << this->balance; 
    return out;
}

bool Accountv2::operator+=(const double amount){
...
bool Accountv2::deposit(double amount) {
    if(amount < 0){
...
bool Accountv2::withdraw(double amount){
    if(balance-amount>=0){
        this->balance-=amount;
...
double Accountv2::get_balance() const {
    return balance;
}